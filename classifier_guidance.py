...
def classifier_call(
        ...
        **kwargs,
        ...
        #6.3 classifier guidance (sort of)
        if guidance_loss_scale!=0 and len(src_image_list)!=0 and len(src_text_list)!=0:
            src_mid_block_output_list=[]
            for src_image,src_text in zip(src_image_list,src_text_list):
                src_tensor=pil_to_tensor_process(src_image).unsqueeze(0)
                src_latents=self.vae.encode(src_tensor).latent_dist.sample()
                small_t=self.scheduler.timesteps[-2]
                print("small_t",small_t)
                noise=torch.randn(src_latents.size())
                noisy_src_latents=self.scheduler.add_noise(src_latents, noise, small_t)
                text_embedding=self._encode_prompt(src_text, device, 1, True, negative_prompt)
                noise_pred = self.unet(noisy_src_latents, small_t, encoder_hidden_states=text_embedding).sample
                src_mid_block_output=self.unet.mid_block.output
                src_mid_block_output_list.append(src_mid_block_output)
